...
mem_widget = lain.widgets.mem({
    settings = function()
    fh = assert(io.popen("upower -i /org/freedesktop/UPower/devices/battery_BAT1 | grep percentage | cut -d' ' -f14,15", "r")) 
        widget:set_markup(space3 .. fh:read("*l") .. markup.font("Tamsyn 4", " "))
    end
})
